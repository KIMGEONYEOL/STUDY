...
-- 한글 1글자는 2바이트임 : 오라클 정품일 때
select length('ORACLE'), length('오라클'),
        lengthB('ORACLE'), lengthB('오라클')
from dual;
-- 확인 : 한글 1 글자가 3 바이트임 (XE 모듈)
...
-- 데이터베이스에서는 계산할 값에 NULL이 있으면, 결과도 NULL 임.
-- 계산할 값이 NULL 이면, 결과값이 나오게끔 NULL을 다른 값으로 바꾸면 됨.
-- 관련 함수 이용함 : NVL(컬럼명, NULL일 때 바꿀 값)
SELECT EMP_ID, EMP_NAME, SALARY, BONUS_PCT,
            (SALARY + (SALARY * NVL(BONUS_PCT, 0))) * 12
FROM EMPLOYEE;

-- 별칭 (ALIAS)
SELECT EMP_ID AS 사번, EMP_NAME AS 이름, SALARY 급여, BONUS_PCT AS "보너스 포인트",
            (SALARY + (SALARY *SALARY + NVL(BONUS_PCT, 0))) * 12 AS "보너스 포함 연봉"
FROM EMPLOYEE;
...
-- 별칭에 숫자, 공백, 기호가 포함되면 반드시 "" 로 묵어야 함. "별칭"
-- 별칭에 사용되는 글자수는 제한없음.
-- 영어 별칭은 기본 대문자로 표시됨 => "소문자", "대소문자 혼합" 따옴표 사용함
...
=> 테이블을 찾아서, 테이블에 저장된 값들 중에서 조건을 만족하는 값이 있는 행(가로줄)을 골라냄
...
-- 직원 중 급여를 4백만 보다 많이 받는 (4백만을 초과하는) 직원 명단 조회
SELECT EMP_ID 사번, EMP_NAME 이름, SALARY 급여
FROM EMPLOYEE
WHERE SALARY > 4000000;

-- 90번 부서에 근무하는 직원 중에서 급여가 2백만을 초과하는 직원 정보 조회
-- 사번, 이름, 급여, 부서코드 : 별칭 처리
SELECT EMP_ID 사번, EMP_NAME 이름, SALARY 급여, DEPT_ID 부서코드
FROM EMPLOYEE
WHERE DEPT_ID = '90' AND SALARY > 2000000; -- 결과행 : 3개

-- 90 또는 20번 부서에 근무하는 직원 조회
-- 사번, 이름, 주민번호, 전화번호, 부서코드: 별칭처리
-- 부서코드로 오름차순정렬 처리함
SELECT EMP_ID 사번, EMP_NAME 이름, EMP_NO 주민번호, DEPT_ID 부서코드
FROM EMPLOYEE
...
FROM EMPLOYEE
--WHERE HIRE_DATE >= '1995/01/01' AND HIRE_DATE <= '2000/12/31';  -- 결과 : 7행
...
-- WHERE 절에서 비교값 여러개를 한 개의 값으로 만들 때 사용하기도 함.
...
SELECT EMP_NAME 이름, HIRE_DATE || ' 입사' 입사일, DEPT_ID 부서코드, SALARY || '(원)' 급여, '기혼' AS 결혼여부 
FROM EMPLOYEE
WHERE HIRE_DATE > '2000/01/01' AND  MARRIAGE = 'Y'; -- 결과 : 13명

-- BETWEEN AND 연산자
-- WHERE 컬럼명 BETWEEN 작은 값 AND 큰 값 ;
-- 컬럼에 기록된 값이 작은 값 이상이면서 큰 값 이하인 값이라는 의미임.
-- WHERE 컬럼명 >= 작은값 AND 컬럼명 <= 큰 값 과 같음
-- 날짜 데이터에도 사용할 수 있음.
...
FROM EMPLOYEE
--WHERE EMP_NO LIKE '_______2%' OR EMP_NO LIKE  '_______4%'; -- 8 명
WHERE SUBSTR(EMP_NO, 8, 1) IN ('2', '4');


-- ESCAPE OPTION
-- WHERE 컬럼명 LIKE '문자패턴 안에 기호문자 사용' ESCAPE '기호문자'

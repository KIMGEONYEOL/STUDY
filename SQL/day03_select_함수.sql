...
-- SELECT 절에서 그룹함수와 단일행 함수는 같이 사용 못 함
...
-- 예 : 직원들 중에서 직원 전체 급여의 평균보다 많은 급여를 받는 직원 조회.
SELECT *
FROM EMPLOYEE
...
-- 소속부서가 50 또는 90 또는 NULL 인 직원들의 보너스 포인트 평균 조회
...
-- 부서 코드가 50 또는 90인 직원들의 
-- 직급코드(CHAR)의 최대값, 최소값
-- 입사일(DATE)의 최대값, 최소값
-- 급여(NUMBER)의 최대값, 최소값 조회

SELECT MAX(JOB_ID) "직급코드 최대값", MIN(JOB_ID) "직급코드 최소값", 
           MAX(HIRE_DATE) "입사일 최대값", MIN(HIRE_DATE) "입사일 최소값",
           MAX(SALARY) "급여 최대값", MIN(SALARY) "급여 최소값"
FROM EMPLOYEE
...
SELECT COUNT(*) -- 전체 행 갯수 : 8
          ...
          ,COUNT(DISTINCT DEPT_ID) -- 중복 제외한 행 갯수 : 1
FROM EMPLOYEE
WHERE DEPT_ID = '50' OR DEPT_ID IS NULL;
...
-- INSTR('문자열리터럴' | 문자가 기록된 컬럼명, 찾을 문자, 찾을 시작위치, 몇 번째 문자)
-- 찾을 문자의 위치를 리턴 (앞에서 부터의 순번)
...
SELECT '               123xxzzyzORACLExxyz567      ',
            LTRIM('               123xxzzyzORACLExxyz567      '), -- 기본 제거문자는 공백문자임. ' '
            LTRIM('               123xxzzyzORACLExxyz567      ', ' '),
            LTRIM('               123xxzzyzORACLExxyz567      ', ' 0123456789'),
            LTRIM('               123xxzzyzORACLExxyz567      ', ' xyz1234567')
FROM DUAL;

SELECT '               123xxzzyzORACLExxyz567      ',
            RTRIM('               123xxzzyzORACLExxyz567      '), -- 기본 제거문자는 공백문자임. ' '
            RTRIM('               123xxzzyzORACLExxyz567      ', ' '),
            RTRIM('               123xxzzyzORACLExxyz567      ', ' 0123456789'),
            RTRIM('               123xxzzyzORACLExxyz567      ', ' xyz1234567')
FROM DUAL;

-- TRIM(LEADING | TRAILING | BOTH '제거할 문자 하나' FROM '문자리터럴' | 문자가 기록된 컬럼명)
-- 기본은 BOTH (앞 뒤 모두 제거)
-- 제거할 문자 생략은 기본 ' '(공백문자임)

SELECT 'aaORACLEaa',
           TRIM('a' FROM 'aaORACLEaa'),
           TRIM(LEADING 'a' FROM 'aaORACLEaa'), -- 왼쪽의 해당 문자 제거
           TRIM(TRAILING 'a' FROM 'aaORACLEaa'), -- 오른쪽의 해당 문자 제거
           TRIM(BOTH 'a' FROM 'aaORACLEaa') -- 앞 뒤의 문자 제거
FROM DUAL;
...
SELECT 'ORACLE 13C',
           SUBSTR('ORACLE 13C', 5), -- LE 18C 
           SUBSTR('ORACLE 13C', 8, 2), -- 18
           SUBSTR('ORACLE 13C', -7, 3) -- CLE
FROM DUAL;

-- 직원 정보에서 주민번호의 생년, 생월, 생일을 각각 분리 조회
SELECT SUBSTR(EMP_NO, 1, 2) 생년,
...
-- SUBSTRB('문자리터럴' | 문자가 기록된 컬럼명, 추출할 바이트 위치, 추출할 바이트 크기)
SELECT 'ORACLE',
...
-- INITCAP('영문리터럴' | 영문자가 기록된 컬럼명) : 단어의 첫 글자만 대문자로 바꾸는 함수
...
-- 주민번호는 생년월일만 보이게하고, 뒷자리는 '*' 처리함 : 891225-*******

 -- day03_selcet_함수2.sql
 
 -- 숫자 처리 함수 ---------------------------------------------
 -- ROUND(), TRUNC(), FLOOR(), ABS(), MOD()
 
 -- ROUND(숫자 | 숫자가 기록된 컬럼명 | 계산식, 반올림할 자릿수)
 -- 버려지는 값이 5 이상이면 자동 반올림함.
 -- 반올림 할 자릿수가 양수이면 소수점 아래 표시(출력)할 자리를 의미함.
 -- 반올림 할 자릿수가 음수이면 소수점 앞(정수부) 올림할 자리를 의미함.
...
SELECT FLOOR(ABS(HIRE_DATE - SYSDATE)) 근무일수, 
 ...
 -- 데이터 딕셔너리에 저장되는 정보는 조회는 할 수 있음 (손댈수는 없음)
 -- 테이블 형태로 각 정보들을 보여줌
 ...
 -- 직원들 중에서 근무년수가 20년 이상된 직원 명단 조회
...
FROM EMPLOYEE;
 ...
 -- NEXT_DAY('날짜' | 날짜가 기록된 컬럼명, '요일 이름')
 -- 지정한 날짜 뒤쪽 날짜에서 가장 가까운 지정 요일의 날짜를 리턴함
 -- 현재 DBMS 의 사용 언어가 'KOREAN' 이므로, 요일 이름에 한글 사용해야 함.
 -- 영어 요일이름 사용하면 에러남
...
SELECT SYSDATE, NEXT_DAY(SYSDATE, 'Sunday') -- 영어 요일이름 사용시 에러 발생
 ...
 -- 추출할 정보 : YEAR, MONTH, DAY, HOUR, MINUTE, SECON

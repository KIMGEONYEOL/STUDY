...
-- WHERE EMP_ID = '100';
-- WHERE TO_NUMBER(EMP_ID) = 100; => CHAR->NUMBER 변경 후 NUMBER와 비교.

SELECT FLOOR(MONTHS_BETWEEN(SYSDATE, '00/12/31')) -- DATE - CHARACER (자동형변환)
FROM DUAL; -- DATE - DATE 연산임.

-- 자동형변환이 안 되는경우가 있음.
SELECT SYSDAY - '15/03/25' -- DATE CHARACTER
FROM DUAL;
...
-- TO_CHAR() 함수 ---------------------------------------------------
-- NUMBER(숫자 : 정수, 실수)나 DATE(날짜, 시간)에 대해서 출력 포멧(FORMAT)을 지정하는 함수
...
-- 날짜에 포멧 적용
-- TO_CHAR(TO_DATE('날짜문자열') | 날짜가 기록된 컬럼명 | DATE 타입반환함수식,  '적용할 포멧문자들 나열')
-- 년월일 시분초 요일 분기 등을 출력 처리 형식으로 지정할 수 있음.

-- 년도 출력 포멧
SELECT SYSDATE, -- NLS_DATE_FORMAE : RR/MM/DD
...
SELECT  EMP_NAME 이름
            , TO_CHAR(HIRE_DATE, 'RRRR"년" fmMM"월" DD"일("DY") 입사"') 입사일
            , TO_CHAR(HIRE_DATE, 'RRRR"년" MON fmDD"일("DY") 입사"') 입사일
FROM EMPLOYEE; -- fm 모델은 포멧 안에서 한 번만 사용할 수 있음.
...
-- RR 와 YY 의 차이
-- 두 자리 년도를 네 자리로 바꿀 때
-- 현재 년도 (24 : 50보다 작음) 일 때
-- 바꿀 년도가 50 미만이면 현재 세기(2000)가 적용 : '14'이면 'RRRR' = 2014 가 됨
-- 바꿀 년도가 50 이상이면 이전 세기(1900)가 적용 : '95'이면 'RRRR' = 1995 가 됨

SELECT HIRE_DATE, -- 년도 두 자리 표시
            TO_CHAR(HIRE_DATE, 'RRRR'),
            TO_CHAR(HIRE_DATE, 'YYYY')
FROM EMPLOYEE; -- 년도를 2자리에서 4

-- 현재 년도(24) 와 바꿀 년도가 둘 다 50 미만이면, Y | R 아무거나 사용해도 됨.
...
-- 현재 년도(24년) 가 50미만이고, 바꿀 년도가 50 이상일 때
...
-- 해당 컬럼에 값이 있으면 바꿀값1로 바꾸고, NULL 이면 바꿀값2로 변경.

-- 직원 정보에서 보너스포인트가 0.2 미만이거나 NULL 인 직원들 조회
-- 사번, 이름, 보너스포인트. 변경보너스포인트 : 별칭
-- 변경보너스포인트 : 보너스포인트 값이 있으면 0.15로 바꾸고, NULL이면 0.05로 변경

SELECT EMP_ID 이름, EMP_NAME 이름, BONUS_PCT 보너스포인트,
...
/*
사용형식 :
DECODE(계산식 | 컬럼명, 값 제시, 값이 맞으면 선택할 값....., 갑 제시N, 선택값 N)
또는
DECODE(계산식 | 컬럼명, 값 제시, 값이 맞으면 선택할 값....., 갑 제시N, 선택값 N, 모든 제시값이 아닐 때 선택할 값)
...
-- 2. CASE 표현식 사용한 경우 : JAVA 의 다중 IF문과 같은 동작 구조를 가짐.
SELECT EMP_ID, JOB_ID,
              TO_CHAR(SALARY, 'L99,999,999'),
           TO_CHAR(CASE JOB_ID
...
작성 위치 : SELECT 구문 각장 마지막에 작성함
실행 순서 : 가장 마지막에 작동됨.
정렬 기준 : SELECT 절의 컬럼명, 별칭, SELECT 절에 나열된 항목의 순번(1, 2, 3...)
정렬 방식 : ASC(생략 가능) | DESC
        ASCending : 오름차순정렬, DESCending : 내림차순정렬        
*/

-- 직원 정보에서 부서 코드가 50 또는 NULL 인 직원들 조회
-- 이름, 급여
-- 급여기준 내림차순정렬하고, 같은 급여에 대해서는 이름기준 오름차순 정렬.

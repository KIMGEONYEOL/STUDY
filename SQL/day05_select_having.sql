...
-- 부서별 급여합계 중 가장 큰 값에 대한 부서코드와 급여 합계를 조회
...
-- 직원 정보에서 급여를 많이 받는 순으로 순위를 매긴다면(큰 값이 1등 : 내림차순)
-- 이름, 급여, 순위
...
-- 2. 지정하는 값이 해당 컬럼 안에서 몇 등인지 순위를 조회 용도로 사용
...
-- ROWNUM
-- ROWID 와 다름
-- ROWNUM 은 SELECT 문 실행시 결과행에 부여되는 행 순번임. (1부터 시작)

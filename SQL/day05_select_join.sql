...
-- ==> EQUAL 이 아닌 앖에 대한 행은 조인에서 제외됨.
-- 연결할 두 테이블의 FOREIGN KEY(외래키, 외부키)로 관계(Relationship)를 가진 컬럼이 조인에 기본적으로 사용됨.

-- 오라클 전용 구문 : 오라클에서만 사용함
-- FROM 절에 조인할(합칠) 테이블들을 나열함
-- WHERE 절에 연결할 컬럼에 대한 조건을 설정함.
-- 단점 : WHERE 절에 입반 조건과 조인 조건이 섞임 = 복잡해짐

SELECT *
FROM EMPLOYEE, DEPARTMENT
WHERE EMPLOYEE.DEPT_ID = DEPARTMENT.DEPT_ID;
-- 결과행 : 20행, EMPLOYTEE.DEPT_ID 가 NULL 인 직원 2명이 제외됨
-- EQUAL INNER JOIN 이라고 함.

SELECT * FROM DEPARTMENT;

-- 오라클 전용구문은 조인한 결과에 두 테이블의 컬럼이 모두 포함되어 있음
-- SELECT 시에 테이블명.컬럼명으로 중복된 컬럼을 구분해 줘야 함.
...
-- 오라클 전용구문과 다른 점 : WHERE 절에서 JOIN 조건을 분리함.

SELECT *
FROM EMPLOYEE
JOIN DEPARTMENT USING (DEPT_ID);
-- JOIN 에 사용된 컬럼이 JOIN 결과에 한 개 존재함, 맨 앞에 표시됨 : 오라클 전용 구문의 조인 결과과 다른 점임.
-- 한 개 존재하기 때문에, 겹치지 않아 따로 구별해주지 않아도 사용 할 수 있음.


SELECT EMP_NAME, DEPT_ID, DEPT_NAME
FROM EMPLOYEE
INNER JOIN DEPARTMENT USING (DEPT_ID); -- 20행
-- JOIN 의 기본은 EQUAL INNER JOIN 임 : 연결 컬럼의 일치하는 값에 대한 행들만 조인됨.
-- INNER 생략해도 됨.
...
SELECT EMP_ID 사번, EMP_NAME 이름, JOB_TITLE 직급명
FROM EMPLOYEE
INNER JOIN JOB USING (JOB_ID); -- 21행

-- OUTER JOIN ---------------------------------------------------------------
-- 기본은 EQUAL INNER JOIN + 값이 일치하지 않은 행도 포함시키는 JOIN
-- OUTER JOIN 도 EQUAL JOIN 임 => 없는 값이 있는 테이블에 값을 추가하는 방식임

-- EMPLOYEE 테이블의 전 직원의 정보를 조인 결과에 포함시키고자 한다면
-- 1. 오라클 전용 구문 : 값이 없는 테이블에 행을 추가하는 방식임 =>  (+) 표시
SELECT *
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_ID = D.DEPT_ID(+);
...
-- 2. ANSI 표준 구문
SELECT *
FROM DEPARTMENT LEFT JOIN EMPLOYEE USING(DEPT_ID);

-- 두 테이블의 제외되는 모든 행을 조인 결과에 포함시킨다면
-- FULL OUTER JOIN 이라고 함

-- 오라클 전용 조인은 FULL OUTER JOIN이 제공되지 않음.
SELECT *
FROM EMPLOYEE E, DEPARTMENT D
...
-- SELF JOIN ----------------------------------------------
-- 같은 테이블을 조인하는 경우
-- 같은 테이블 안의 다른 컬럼을 참조하는 외래키(FOREIGN KEY)가 있을 때 사용함
-- EMP_ID : 직원의 사번 --> MGR_ID : 관리자 사번 <= EMP_ID 컬럼값 가져다가 사용(참조)하는 컬럼임
-- 즉, 관리자는 직원 중에서 존재한다의 의미임.

-- 관리자가 배정된 직원의 명단과 관리자인 직원의 이름을 조회.
-- ANSI 표준구문에서 SELF JONI 시에는 테이블 별칭 사용해야 함 = ON 사용해야 함.
...
-- 직원 전체 조회임
-- 1. ANSI 표준 구문
SELECT EMP_NAME 직원이름, JOB_TITLE 직급명, LOC_DESCRIBE 지역명, COUNTRY_NAME 국가명 
FROM EMPLOYEE
LEFT JOIN JOB USING(JOB_ID)
LEFT JOIN DEPARTMENT USING (DEPT_ID)
LEFT JOIN LOCATION ON (LOCATION_ID = LOC_ID)

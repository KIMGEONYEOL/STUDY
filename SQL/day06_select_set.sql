...
SELECT EMP_ID, ROLE_NAME
FROM EMPLOYEE_ROLE -- 22행
UNION ALL -- 26행 : 중복행 '104 SE' 가 모두 포함됨.
SELECT EMP_ID, ROLE_NAME
FROM ROLE_HISTORY; -- 4행
...
SELECT EMP_ID, ROLE_NAME
FROM EMPLOYEE_ROLE -- 22행
MINUS -- 21행 : 첫번쨰 SELECT 문에서 중복행 '104 SE' 가 1개 제외됨.
SELECT EMP_ID, ROLE_NAME
FROM ROLE_HISTORY; -- 4행

-- sET 연산자 사용시 주의사항 확인 : 
--  1. 두 SELECT 문의 컬럼 갯수 같아야 함
SELECT EMP_NAME, JOB_ID, HIRE_DATE -- 3개
FROM EMPLOYEE
WHERE DEPT_ID = '20'
UNION
--SELECT DEPT_NAME, DEPT_ID -- 2 개
SELECT DEPT_NAME, DEPT_ID, NULL -- 더미 컬럼으로 갯수 맞춤.
FROM DEPARTMENT
WHERE DEPT_ID = '20';

-- 활용 : ROLLUP() 함수의 중간집계 위치에 원하는 문처리 못 함.
...
WHERE EMP_ID != '141' AND DEPT_ID = '50'
--ORDER BY 3, 1;
ORDER BY 구분, 1; -- 같은 별칭을 사용했으면, ORDER BY 구문에 해당 별칭 사용 가능.
...
-- 각 쿼리문의 SELECT 절에 선택한 컬럼명이 동일한 경우에는 조이으로 바꿀 수 있음
-- INTERSECT ==> USING (EMP_ID, ROLE_NAME)
-- INNER JOIN 이 기본이므로 (104 SE) = (104 SE) 조인으 로 행이 연결될 것임
-- (104 SE-ANLY) != (104 SE) : 다르다, 조인에 포함 안 됨.

-- 위의 구문을 조인으로 바꾼다면
SELECT EMP_ID, ROLE_NAME
FROM EMPLOYEE_ROLE
JOIN ROLE_HISTORY USING (EMP_ID, ROLE_NAME); -- 1행
...
-- 주의사항 2 : SELECT 절의 컬럼별 위아래 자료형이 같아야 함.

-- day06_select_subquery.sql

-- SELECT : 서브쿼리 (SUBQUERY) - ------------------------------------
-- SELECT 문 안에 사용되는 SELECT 문 => 내부쿼리라고도 함.
-- 바깥 SELECT 문을 외부쿼리(메인 쿼리, MAIN_QUERY) 라고 함.

/*
        바깥함수(반환값이 있는 함수())
        => 안쪽 함수가 먼저 실행되면서, 반환된 값을 바깥함수가 사용하는 구조.
        
조건구문에서 컬럼명 비교연산자 비교값 <-- 비교값을 알아내기 위한 SELECT 문을 값 대신에 사용할 수 있음
...
-- 서브쿼리의 결과값이 1개인 경우
-- 단일행 서브쿼리 앞에는 일반 비교연산자를 사용 할 수 있음.
...
-- 직원 중에서 전체 급여에 대해 최저 급여(가장 작은 값)을 받는 직원 명단 조회
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
-- WHERE SALARY = MIN(SALARY); -- ERROR : 그룹함수는 WHERE 절과 함께 사용 불가능.
WHERE SALARY = (SELECT MIN(SALARY) -- 1500000
                         FROM EMPLOYEE);

-- HAVING 절에서도 서브쿼리 사용할 수도 있음
-- 예 : 부서별 급여합계 중 가장 큰 값에 대한 부서명과 급여합계 조회
-- 1. 부서별 급여합계 중 최대값 조회
SELECT MAX(SUM(SALARY)) -- 18100000 
FROM EMPLOYEE
GROUP BY DEPT_ID;

-- 2. 최대 급여 총합을 가져가는 부서 조회
...
-- 다중행 서브쿼리 앞에는 일반 비교연산자 (비교값 1개가 원칙) 사용 못 함 : ERROR
...
WHERE SALARY > ANY (SELECT SALARY -- 3행
                                FROM EMPLOYEE
                                JOIN JOB USING (JOB_ID)
                                WHERE JOB_TITLE = '과장')
AND JOB_TITLE = '대리';

-- 컬럼명 > ALL (다중행 서브쿼리) : 가장 큰 값보다 큰 값 선택
-- 컬럼명 < ALL (다중행 서브쿼리) : 가장 작은 값보다 작은 값 선택

-- 예 : 모든 과장들의 급여보다 더 많은 급여를 받는 대리 직원 조회
SELECT EMP_ID, EMP_NAME, JOB_TITLE, SALARY
FROM EMPLOYEE 
JOIN JOB USING (JOB_ID) 
WHERE SALARY > ALL (SELECT SALARY -- 3행
                                FROM EMPLOYEE
                                JOIN JOB USING (JOB_ID)
                                WHERE JOB_TITLE = '과장')
AND JOB_TITLE = '대리';
...
-- 1. 직급별 급여 평균 조회
SELECT JOB_ID, TRUNC(AVG(SALARY), -5) -- 8행
FROM EMPLOYEE
GROUP BY JOB_ID;
...
-- FROM 절에서도 서브쿼리 사용할 수 있음 => 서브쿼리의 결과뷰를 테이블 대신 사용함
...
-- ANSI 표준구문으로 조인시에는 USING 을 사용할 때는 테이블 별칭 사용 못 함.
--         ON 사용시에는 테이블 별칭 사용 가능.
...
-- 상호연관 서브쿼리는 서브퀴리가 메인쿼리의 값을 가져다가 결과를 만듦
-- 즉, 메인쿼리의 값이 바뀌면 서브쿼리의 결과도 달라지게 됨.
-- 서브쿼리를 따로 실행할 수 없음.

-- 자기 지급의 평균 급여를 받는 직원 조회 : 상호연관 서브쿼리로 작성한다면,
SELECT EMP_NAME, JOB_TITLE, SALARY
FROM EMPLOYEE E
LEFT JOIN JOB J ON (E.JOB_ID = J.JOB_ID)
-- WHERE 해당 직원의 SLALRY = (해당 직원의 직급의 평균급여 계산)
-- WHERE 첫번째 직원의 SALARY = (첫번째 직원의 직급을 가져다가 해당 직급의 급여평균을 계산)
-- WHERE 두번째 직원의 SALARY = (두번쨰 직원의 직급을 가져다가 해당 직급의 급여평균을 계산)
-- 22명에 대해 22번 작동될 것임
...
-- 급여 많이 받는 직원 5명 조회 : 인라인뷰 사용하지 않는 경우
SELECT ROWNUM, EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE

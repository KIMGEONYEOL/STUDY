...
-- 테이블에 데이터를 추가 기록 (저장 : INSERT) => 행 갯수 늘어남
-- 테이블에 기록된 데이터(컬럼값)를 수정 (UPDATE) => 행 갯수 영향 없음
-- 테이블에 기록된 행을 삭제 (DELETE) => 행 갯수 줄어듦
...
-- WHERE 절에 비교값 대신에 서브쿼리 사용할 수 있음.
...
-- 예 : 해외영업2팀 직원들의 보너스포인트를 모두 0.3으로 변경하시오.
UPDATE EMP_COPY
SET BONUS_PCT = 0.3
...
-- SET 절에 변경값은 제약조건에 위배되지 않는 값이어야 한다.
ALTER TABLE EMP_COPY
...
/*
작성 형식 : 
INSERT INTO 테이블명 (값 기록할 컬럼명, 컬럼명, 컬럼명 ......)
VALUES (기록할 값, 기록할 값, ......);

VALUES 대신에 서브쿼리 사용해도 됨 (서브쿼리) 괄호 생략가능
INSERT INTO 테이블명
(SELECT 서브쿼리);
...
3. 기록값은 제약조건에 위배되지 않는 값을 사용해야 함.
...
-- 트랜잭션 명령어 ( TCL : Transaction Control Language,  트랜잭션 제어어0=) : COMMIT, ROLLBACK

COMMIT ; -- 입력한 DML 값을 다른 사용자들에게도 보여주기 위해 꼭 넣어줘야 한다.

-- 테이블이 가진 전체 컬럼에 값 기록시에는 컬럼명 생략 할 수 있음.
-- INSERT 시에 기록할 값 대신에 DEFAULT, NULL 사용해도 됨.
SELECT * FROM EMP_COPY; -- 22 행
DESC EMP_COPY; -- 컬럼 12개

INSERT INTO EMP_COPY (EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, HIRE_DATE, JOB_ID, SALARY, BONUS_PCT, 
                                    MARRIAGE, MGR_ID, DEPT_ID)
VALUES ('900', '홍길동', '891225-1234567', 'hong123@test.org', '01012345678', '20/06/12', 'J5', 4500000, 0.05,
            DEFAULT, '176', NULL);
            
SELECT * FROM EMP_COPY; -- 23 행
...
VALUES ('84', '하지언', '880123-2346788', 'ha_83@test.org', '01084547755', '22/03/15', 'J6', 5000000, 0.2, 'Y', '141', '30');

SELECT * FROM EMP_COPY; -- 24 행
COMMIT;
...
VALUES('888', '신예찬', '921112-1245252', DEFAULT, DEFAULT);
SELECT * FROM EMP_COPY; -- 25 행
SELECT * FROM EMP_COPY
WHERE EMP_NAME = '신예찬'; -- NULL 입력 확인
...
VALUES (NULL, '홍길순', '911123-2345678'); -- NULL 사용시 에러 : 제약조건 제대로 작동함. (PRIMARY KEY)

INSERT INTO EMP_COPY (EMP_ID, EMP_NAME, EMP_NO)
VALUES ('100', '홍길순', '911123-2345678'); --  ERROR : PRIMARY KEY => 중복값도 입력 불가. '100' 이라는 사번이 중복되어 ERROR.

INSERT INTO EMP_COPY (EMP_ID, EMP_NAME, EMP_NO)
VALUES ('777', NULL, '945323-2345678'); -- ERROR : EMP_NAME의 NOT NULL 제약조건에 위배.

INSERT INTO EMP_COPY (EMP_ID, EMP_NAME, EMP_NO)
VALUES ('777', '홍길순', '621133-1483658'); -- ERROR : EMP_NO의 UNIQUE 제약 조건에 위배.

INSERT INTO EMP_COPY (EMP_ID, EMP_NAME, EMP_NO)
VALUES ('777', '홍길순', 'NULL'); -- ERROR : EMP_NO의 NOT NULL 제약 조건에 위배. NULL 사용 못 함
...
1. FOREIGN KEY 제약조건으로 참조(REFERENCE)되고 있는 테이블의 참조컬럼(부모키)은 삭제 불가능임
    => 외래키 제약조건 지정시 삭제룰 (SET NULL, CASCADE)이 함께 설정된 경우는 삭제 가능함
2. WHERE 절이 생략되면 테이블의 모든 행이 삭제됨. (복구 가능함)

테이블의 모든 행을 삭제하는 명령 구문 : 
TRUNCATE TABLE 테이블명; --> 복구 불가능(롤백 안됨)
*/
...
SELECT * FROM DCOPY; -- 0 행

ROLLBACK; -- DELETE 취소
SELECT * FROM DCOPY; -- 7행 : 복구 됨.

TRUNCATE TABLE DCOPY;
SELECT * FROM DCOPY; -- 0 행
ROLLBACK; -- TRUNCATE 취소
SELECT * FROM DCOPY; -- 0 행 : 복구 안 됨. / DELETE 와 TRUNCATE의 차이점.

...
-- 인덱스 ----------------------------------
-- SQL 구문 (SELECT, DML)의 처리 속도를 향상시키기 위해서 컬럼에 대해 생성하는 데이트베이스 객체임
-- 인덱스 내부 구조는 B* 트리(이진탐색트리 : BST, Binary Search Tree) 임
-- 인덱스 객체는 새성하는 데에 시간이 필요하고, 공갸도 필요함=> 사용한다고 반드시 좋은 것은 아님.
...
-- 인덱스를 생성하는 데 시간이 필요함
-- 테이블에 DML(INSERT, UPDATE, DELETE)이 자주 발생하는 경우, 인덱스 트리 재구축이 자동 진행되므로
-- 성능 저하됨.

-- 인덱스 생성 구문 : 
/*
CREATE  [UNIQUE] INDEX 인덱스이름 ON 테이블명 (컬럼명[, 컬럼명, ......] | 함수계산식);

인덱스 유형 : UNIQUE INDEX, NONUNIQUE INDEX
*/

-- UNIQUE INDEX :
-- 컬럼에 UNIQUE 제약조건 설정한 것과 같음 => 같은 값 두번 기록 못 함 (중복검사)
-- PRIAMARY KEY와 UNIQUE 제약조건이 설정된 컬럼에 대해 자동으로 UNIQUE INDEX가 생성됨.
...
-- 2. 새로운 데이터를 입력 해보고, 오류 원인을 파악하시오.
...
-- 오류 원인 :
-- EMPLOYEE 의 EMP_NAME 컬럼이 '감우섭' 이름이 이미 존재함.
-- UNIQUE INDEX는 UNIQUE 제약조건 기능을 수행함.
...
-- NONUNIQUE INDEX -------------------
-- 빈번하게 사용되는 일반 컬럼에 적요하는 인덱스임
-- 성능 향상을 위해 사용됨
CREATE INDEX IDX_DID
ON EMPLOYEE (DEPT_ID);

-- 인덱스 삭제 ---------------
-- 테이블이 삭제되면, 인덱스도 같이 삭제됨.
-- DROP INDEX 인덱스이름;
DROP INDEX EMP_NAME;
...
-- EMPL 01 의 EMP_ID 컬럼에 대해 UNIQUE INDEX 만드시오. : IDX_EID
-- 검색 속도 비교를 위해 SELECT 쿼리문을 EMP_ID 컬럼으로 조회해보기 : 사번 141번 정보 조회
CREATE TABLE EMPL01
AS
...
SELECT * FROM EMPL01
WHERE EMP_ID = '141' ; -- 0.002 초

SELECT * FROM EMPL02
WHERE EMP_ID = '141'; -- 0.004 초
...
-- 함수 기반 인덱스 -----------------------
-- SELECT 절이나 WHERE 절에 산술 계산식이나 함수계산식 사용하는 경우
-- 계산식은 인덱스의 적용을 받지 않는다. (인덱스가 계산을 수행할 수 없기 때문)
-- 계산 결과값을 키워드로 해서 검색 트리를 구성할 수 있음.

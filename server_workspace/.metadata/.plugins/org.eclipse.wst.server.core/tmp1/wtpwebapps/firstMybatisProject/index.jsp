...
function movePage(){
	...
	location.href = "/firstm/views/member/loginPage.html";
}

</script>
</head>
<body>
<h1>firstmServleProject : firstm</h1>
<hr>
<header>
	<div id="banner" class="lineA">
		<img src="/firstm/resources/images/photo2.jpg">
	</div>
	<% if(loginUser == null){ // 로그인 하지 않은 상태일 때 %>
		<div id="loginBox" class="lineA">
			fristm 사이트 방문을 환영합니다.<br>
			<button onclick="movePage()">fristm 로그인</button><br>
			<%--로그인 버튼을 클릭하면 자바스크립트 movePage() 함수가 실행되게 해서, 로그인 페이지가 나타나게 처리함 --%>
			<a href="/firstm/views/member/enrollPage.html">회원가입</a>
			<%-- 회원가입 클릭하면 회원가입페이지가 연결되어 출력되겍 링크 설정했음 --%>
		</div>
	<% }else{ // 로그인 했을 때 %>
		<div id="loginBox" class="lineA">
			<%= loginUser.getUserName() %> 님 &nbsp; 
			<a href="/firstm/logout">로그아웃</a> <br>
			메일 0, 쪽지 0 
		</div>
	<%} %>
</header>

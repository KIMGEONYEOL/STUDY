...
<title>first</title>
<style type="text/css">
 	table th { background-color: pink; }
...
window.onload = function(){
	...
	photofile.addEventListener('change', function(event){
		...
		reader.onload = (e) => { // e : event 객체
			myphoto.setAttribute('src', e.target.result); // e.targe : 이벤트 발생 대상(읽어들인 파일 정보)
...
function validate(){
	// 수정하기 전송 보내기 암호와 암호 확인이 일치하지 않는지 확인
	...
	if(pwdValue !== pwdValue2){
		...
		document.getElementByID('userPwd').focus(); // 입력 커서 지정함
		return false; // 전송 취소함
	}
	// 변경된 암호의 값 형식이 요구한 대로 구성되었는지 검사
...
<form action="mupdate.do" method="post" onsubmit="return validate();" enctype="multipart/form-data">
	<%-- view 페이지에서는 보이지 않으면서, 서버로 폼 안의 값들이 전송될 때 숨겨서 같이 보내야 하는 값 처리 --%>
	<input type="hidden" name="originalUserPwd" 
		value="<%-- <%= member.getUserPwd() %> --%>${ requestScope.member.userPwd }">
	<input type="hidden" name="ofile" 
	value="<%-- <%= request.getAttribute("ofile") %> --%> ${ requestScope.ofile }">
<table id="outer" align="center" width="700" cellspacing="5" cellpadding="0">
	...
	<tr><th width="120">*아이디</th><td>
				...
				value="<%-- <%= member.getUserId() %> --%>${ requestScope.member.userId }" readonly>
			</td></tr>
	<tr><th>사진첨부</th>
		...
		<td>
			...
			<div id="myphoto" style="margin:0;width:150px;height:160px;padding:0;border:1px solid navy;">
				...
				<img src="/first/resources/photo_files/${ requestScope.member.photoFileName }
				<%-- <%= member.getPhotoFileName() %> --%>" id="photo" 
	...
	<tr><th>*이	름</th><td><input type="text" name="userName" id="userName" 
	...
	<tr><th>*나	이</th><td><input type="number" name="age" min="19" 
	value="<%-- <%= member.getAge() %> --%>${ requestScope.member.age }" ></td></tr>
	<tr><th>*전화번호</th><td><input type="tel" name="phone" 
	value="<%-- <%= member.getPhone() %> --%>${ requestScope.member.phone }" ></td></tr>
	<tr><th>*이메일</th><td><input type="email" name="email" 
	value="<%-- <%= member.getEmail() %> --%>${ requestScope.member.email }"></td></tr
	>	<tr><th colspan="2">
			<input type="submit" value="수정하기"> &nbsp;
			<%-- <a href="mdelete.do?userid= --%>
			<c:url var="mdel" value="mdelete.do">
				<c:param name="userId" value="${ requestScope.member.userId}"/>
			</c:url>
			<a href="${ mdel }"><%-- <%= member.getUserId()%> --%>탈퇴하기</a> &nbsp;
			</th>	
	</tr>
</table>
</form>

...
public class ReplyController {
    ...
    public String replyInsertMethod(
        ...
        if (replyService.insertReply(reply) != null) {
            ...
            model.addAttribute("message", reply.getBoardRef() + "번 글에 대한 댓글 등록 실패!");
            return "common/error";
        }
    } // breply.do
    ...
    public String replyUpdateMethod(Reply reply, Model model) {
        ...
        if (replyService.updateReply(reply) != null) {
            //  댓글과 대댓글 수정 성공시 다시 상세보기가 보여지게 처리함
            return "redirect:bdetail.do?bnum=" + reply.getBoardRef();
        } else {
            model.addAttribute("message", reply.getBoardRef() + "번 글 수정 실패!");
            return "common/error";
        }
    }

    // 댓글과 대댓글 삭제 처리용
    @RequestMapping(value = "breplydelete.do", method = RequestMethod.POST)

...
public class BoardService {
	...
	public ArrayList<Board> selectTop3(){
		//jpa 가 제공하는 findAll() 로 모두 조회해 와서, 3개만 추출하는 방법이 있을것임
	...
	public Board selectBoard(int boardNum) {
		// jpa 가 제공하는 메소드 사용 : findById(id로 지정한 프로퍼티  변수의 값) : Optional<Entity> 리턴
	...
	public int updateAddReadCount(int boardNum) {
		// jpa 가 제공하는 메소드 사용 : save(Entitly) : 성공 Entity, 실패(에러) null
		// update 쿼리문 : 전달되는 Entity 의 id 로 지정된 프로퍼티의 값이 존재하면 update 가 작동됨
		// jpa 의 save() 는 수정할 정보를 가진 엔티티를 넘겨줘야 함
		Optional<BoardEntity> optionalBoard = boardRepository.findById(boardNum);
		BoardEntity entity = optionalBoard.get();
		log.info("BoardService boardEntity : " + entity); // DB 에서 조회된 게시글 정보 확인
	...
	public int insertBoard(Board board) {
		...
		log.info("BoardService board insert : " + board.getBoardNum());
		// jpa 가 제공하는 메소드 사용 : save(Entity)
		// => pk 에 해당되는 글 번호가 테이블에 없으면 insert 문 실행함
	...
	public int selectSearchTitleCount(String keyword) {
		...
		// 추가 작성해서 사용 : Repository Interface 에 추가 작성함

...
public class ReplyService {
    ...
    public Object insertReply(Reply reply) {
        ...
        if(reply.getReplyLev() == 2){   // 대댓글 등록이면
            ...
            // 같은 원글, 같은 레벨인 대댓글의 마지막 순번 조회 : 추가한 메소드 사용
            reply.setReplySeq(replyRepository.findLastReplySeq(reply.getBoardRef(),
                    reply.getReplyReplyRef(), reply.getReplyLev()) + 1);
        }

//        // 댓글 순번 기록을 위해 마지막 순번 조회
//        int boardRef = reply.getBoardRef();
//        int replyReplyRef = reply.getReplyReplyRef();
//        int replyLev = reply.getReplyLev();

        return replyRepository.save(reply.toEntity());  // jpa 가 제공하는 메소드
    ...
    public int deleteReply(int replyNum){
        // 댓글, 대댓글 삭제
        // 댓글 삭제시 제약조건에 의해 대댓글도 함께 삭제됨(on delete casecade 설정되어 있기 때문)
        // jpa 제공 메소드 사용
        try{

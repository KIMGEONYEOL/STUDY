...
//댓글의 댓글달기 버튼 클릭시 실행함수
function requestReplyReply(replyReplyRef){
	location.href = "/first/rreplyform.do?boardRef=" + ${board.boardNum} + "&replyReplyRef=" + replyReplyRef;
}
//원글 삭제하기 버튼 클릭시 실행함수
...
<!-- 해당 게시글의 댓글과 대댓글 출력 -->
<div>
	<table align="right" width="500" border="1" cellspacing="0" cellpadding="5">
		...
		<tr><th>작성자</th><td>${ reply.replyWriter }</td>
			<th>등록날짜</th>
			<td><fmt:formatDate value="${ reply.replyDate }" pattern="yyyy-MM-dd" /></td>
		</tr>
		<tr></tr>
		<tr><th>내 용</th><td colspan="3">${ reply.replyContent }</td></tr>
		<tr><th colspan="4">
			<%-- 로그인한 경우 표시되게 함 --%>
			<c:if test="${ !empty sessionScope.loginUser }">
				<%-- 본인이 작성한 글 또는 관리자이면 수정, 삭제 버튼 제공함 --%>
				<c:if test="${ loginUser.userId eq reply.replyWriter or loginUser.adminYN eq 'Y' }">
					...
					<button onclick="requestReplyDelete(${reply.replyNum}); return false;">삭제</button>
				</c:if>

				<%-- 본인이 작성한 글이 아니면 또는 관리자이면 댓글달기 버튼 제공함 --%>
				<c:if test="${ loginUser.userId ne reply.replyWriter or loginUser.adminYN eq 'Y' }">
					<c:if test="${ reply.replyLev eq 1 }">
						<button onclick="requestReplyReply(${reply.replyReplyRef}); return false;">댓글달기</button>
					</c:if>
				</c:if>
			</c:if> &nbsp;
		</th></tr>

...
// 값(data)을 저장하는 목정의 클래스

// 작성 규칙 : 
// 1. 반드시 캡슐화 해야함. : 모든 Field 가 private 할 것
// 2. 기본생성자와 매개변수 있는 생성자를 가지고 있어야 함.
// 3. 모든 필드에 대한 getter와 setter 가 있어야 함.
// 4. toString(), equlas() 등에 대한 오버라이딩 해야 함(선택사항)
// 5. 반드시 직렬화 할 것 : 네트웤 입출력을 위해 객체 인스턴스를 1바이트 크기로 일렬로 나열하는 것
...
public class Person implements java.io.Serializable{ 
	...
	// 후손이 가진 필드를 다루려면, 부모 타입에서 후손 타입으로 형변환해야 함
	// (후손클래스명)부모타입레퍼런스

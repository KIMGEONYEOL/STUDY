...
public class Point {
	...
	public Point() {}	// jvm 이 준비해 놓은 기본값으로 초기화됨
	// 정수 : 0, 실수 : 0.0, boolean : false, char : '\u0000', 레퍼런스 타입 : null
	...
	public Point(double x, double y) {
		// 생성자 안에는 this 레퍼런스(객체 주소 저장 변수)가 존재함
		// new 할 때 자유메모리 heap 에 할당되는 객체의 주소를 자동으로 전달받음
		System.out.println("Point 기본생성자 this :" + this.hashCode());
		// + hashCode() : int => 메모리에 할당된 객체 인스턴스의 주소를 10진수로 리턴
		// java.lang.Object 클래스가 소유한 메서드임 => 모든 클래스는 Object 의 후손임
		this.x = x;
		this.y = y;
	...
	// 일반 메서드
	// 오버라이딩(ovderriding) : 부모 메서드를 후손이 재작성하는 것
	// 주의 : 부모 메서드의 시그니쳐(signiture) 그대로 사용해야 함/ 메서드 안의 실행내용만 변경함
	@Override // annotation : 컴파일러용 주석(comment), 컴파일러 알림용
	public String toString() {
		// Object 클래스 : this 가 참조하는 :클래스명@16진수 헤시코드값" 리턴
		// 오버라이딩 구현 변경 : this 가 참조하는 인스턴스 안의 필드값 출력 확인용 문장 리턴
	...
	public boolean equals(Object obj) {
		// Object 클래스의 원래 기능 : return this == obj: => 두 주수가 같은지 비교
		// 두 레퍼런스가 참조하는 객체 안의 필드 값이 같은지로 변경함
		
		// 메서드 사용시 Point 객체의 주소를 전달하면, Object 타입의 매개변수가 받게 됨 : 다형성 적용됨
		// 그런데 Object obj 는 Point 가 가진 x 에 접근할 수 없음 : obj 에는 x가 없음
		// 다형성 : 부모 클래스형 레퍼런스가 후손들의 주소를 받는 것을 말함
		// 클래스 형변환 필요함 : Object 에서 Point 형으로 바꿈 => 부모와 후손간에만 형변환 가능함
		Point pobj = (Point)obj;
		return (this.x == pobj.x) && (this.y == pobj.y);	
	
	}
	// 오버라이딩 규칙 2 : 부모 메서드의 접근제한자를 더 넓은 범위의 접근자로 변경할 수 있음
	// 오버라이딩 규칙 3 : 부모 메서드의 예외는 갯수를 줄일 수 있음
	@Override
	public Object clone() {
